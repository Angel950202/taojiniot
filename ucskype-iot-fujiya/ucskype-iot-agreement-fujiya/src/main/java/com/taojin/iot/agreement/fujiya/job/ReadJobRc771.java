...
@Component("readJobRc771")
@Lazy(false)
public class ReadJobRc771 {
	private static Logger logger = LoggerFactory
			.getLogger(ReadJobRc771.class);
	public static int jobState = 0;
	
	/**
	 * 定时处理指令部分
	 */
	@Scheduled(cron = "${job.readJobRc771.cron}")
	public void evictExpired() {
		JedisUtil jedisUtil = new JedisUtil();
		Hash hash = jedisUtil.HASH;
		String jobState = hash.hget("iot_dtu_state", "RC771");
		logger.info("[定时任务]---->RC771,jobState={}",jobState);
		if(StringUtils.isBlank(jobState)){
			hash.hset("iot_dtu_state", "RC771", "0");
			jobState = "0";
		}
		logger.info("定时抄读检索...jobState={},type={}",jobState,"RC771");
		if(StringUtils.equalsIgnoreCase(jobState, "1")){
			logger.info("定时抄读检索,跳过,当前任务执行中...jobState={},type",jobState,"RC771");
			return;
		}
...
//			hash.hset("iot_dtu_state", "RC771", "1");
			...
			agreementRc701Service.readByTimer(AgreementFujiyaEnum.RC771);
		}catch(Exception e){
			logger.info("定时抄读检索,执行失败,错误原因={},type={}",e.getMessage(),AgreementFujiyaEnum.RC771.getDesc());
			e.printStackTrace();
		}
		logger.info("定时抄读检索完成...type=RC771");
	}
}
