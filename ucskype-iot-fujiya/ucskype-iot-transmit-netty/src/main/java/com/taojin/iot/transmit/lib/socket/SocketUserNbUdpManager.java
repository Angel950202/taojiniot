 ...
 *
 */
public class SocketUserNbUdpManager extends Server{
	
	private SocketServer socketServer;

	private static final Logger logger = LogManager.getLogger(SocketUserNbUdpManager.class);
	...
	public void start() {
		...
		try {
			socketServer.startUserNbUdp();
			serverStatus = ServerStatus.STARTED;
		} catch (Exception e) {
			logger.info("Socket USER-NB-UDP" + socketServer.getPort() + "端口服务启动失败");
			e.printStackTrace();
		}
