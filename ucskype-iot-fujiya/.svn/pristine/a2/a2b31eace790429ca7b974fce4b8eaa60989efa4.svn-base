 ...
 *
 */
public class ServerUdpHandler extends
	...
	public ServerUdpHandler(MessageHandle messageService) {
	...
	public String handlerPackage(String sessionId, String msg) {
		...
		if (nowSessionMap.startsWith("fefe68")
				&& nowSessionMap.endsWith("16")) {// 完整包
	...
	protected void channelRead0(ChannelHandlerContext ctx, DatagramPacket packet)
		...
		logger.info("【UDP】收到消息: " + msgString);
		logger.info("【UDP】收到消息: " + msgWoshou);
		messageService.receiveBytes(ctx.channel().id().toString(), CommunicatType.SOCKETUDP, msgString.getBytes(),ctx);
	}
